...
import 'package:gewerber_backend_commercial_server/src/generated/modules/subscription/models/subscription.dart'
    as _i7w979jt;
import 'package:serverpod/serverpod.dart' as _is;
import '../modules/subscription/services/cancel_subscription_future_call.dart'
    as _ipusfnz0;
...
class FutureCalls extends _is.FutureCallDispatch<_FutureCallRef> {
  ...
  void initialize(
    ...
    var registeredFutureCalls = <String, _is.InvokableFutureCall>{
      'CancelSubscriptionCancelFutureCall':
          CancelSubscriptionCancelFutureCall(),
    };
...
class _FutureCallRef {
  ...
  late final cancelSubscription = _CancelSubscriptionFutureCallDispatcher(
    _invokeFutureCall,
  );
}

class _CancelSubscriptionFutureCallDispatcher {
  _CancelSubscriptionFutureCallDispatcher(this._invokeFutureCall);

  final _InvokeFutureCall _invokeFutureCall;

  Future<void> cancel(_i7w979jt.Subscription subscription) {
...
///
/// No-ops when the row disappeared, was already canceled, or no longer
/// carries `cancelAtPeriodEnd` (e.g. an admin reactivated it). Otherwise
/// cancels at the provider when a PayPal id is present, marks the row
/// `canceled`, invalidates the entitlement cache and writes a
/// `subscription.canceled_at_period_end` audit row.
class CancelSubscriptionCancelFutureCall
    extends _is.FutureCall<_i7w979jt.Subscription>
    implements _is.InvokableFutureCall<_i7w979jt.Subscription> {
  @override
  _ida.Future<void> invoke(
    _is.Session session,
    _i7w979jt.Subscription? subscription,
  ) async {
    await _ipusfnz0.CancelSubscriptionFutureCall().cancel(
      session,
      subscription!,
    );
  }
